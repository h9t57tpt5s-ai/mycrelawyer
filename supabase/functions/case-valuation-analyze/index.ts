  ...
  try {
    ...
    const analysisSchema = {
      ...
      properties: {
        ...
        damagesRange: nullableRangeSchema("YOUR OWN independent probability-weighted net exposure/recovery range for the filing party, in dollars (low/high) -- informed by the baseline but not bound by it. Never a single point estimate. MUST be null -- not a placeholder or illustrative range -- if the case materials contain NO actual economic anchor at all (no rent/lease-value figure, no stated damages amount, no dollar figure of any kind tied to the specific dispute). If the case materials show a specific dollar amount actually requested from the court by that side (a motion, demand, or pleading with a stated prayer), this range's high end must never exceed that requested amount -- see the REQUESTED-RELIEF CEILING instruction."),
    ...
    const analysisStream = anthropic.messages.stream({
      ...
      system:
        ...
        "REQUESTED-RELIEF CEILING: if the case materials include or describe a specific motion, pleading, or demand that states a specific dollar amount actually being requested/prayed for from the court (e.g. a motion for summary judgment asking for a stated total, a demand letter with a stated figure, a petition with a specific prayer rather than an open-ended 'to be proven at trial'), your damagesRange and bestGuessValue for the side making that request must NEVER exceed that stated amount, even if your own analysis of the underlying facts would support a larger recovery. Reasoning that a bigger number is legally defensible belongs in the analysis prose as an explicit note (e.g. 'the record separately supports an additional $X not included in this motion's request') -- it must never be folded into the numeric damagesRange/bestGuessValue themselves. This mirrors real practice: a party's own choice about what to formally put in front of a court is real information about the state of that specific proceeding, not a ceiling this tool gets to second-guess upward. This ceiling applies only to the side that made the request and only up to what THAT request actually seeks -- it doesn't cap the other side's claims, and it doesn't apply when the operative pleading seeks unliquidated damages 'to be proven at trial' with no specific number attached (there, your own analysis sets the range as usual). " +
