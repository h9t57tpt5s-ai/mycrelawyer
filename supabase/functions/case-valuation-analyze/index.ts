    ...
    const issuesRaw = Array.isArray(analysisParsed.issues) ? analysisParsed.issues.map((iss: Record<string, unknown>) => {
      ...
      if (figures.length) {
        if (amt?.basis === "competing") {
          ...
          // v4.2 (2026-09-25): back to v4's low end. v4.1 made an itemized
          // claim's range the full sum, to stop the "disputed" flag moving
          // the low end between runs; a free replay of the saved v4 runs
          // showed that cost two correct ranges, while the flag's swings had
          // never changed a hit-or-miss verdict or the best guess.
      ...
      return {
        ...
        damagesRange,
        supportedCeiling,
        // Saved with each result so formula changes can be replayed from
        // stored runs without new API calls (Jeff, 2026-09-25, on cost).
        basis: amt?.basis === "competing" ? "competing" : (figures.length ? "itemized" : null),
        figures,
        droppedFigures,
