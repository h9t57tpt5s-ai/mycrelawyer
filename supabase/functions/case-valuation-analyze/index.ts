...
function logUsage(
  label: string,
  model: string,
  usage: { input_tokens?: number | null; output_tokens?: number | null; cache_read_input_tokens?: number | null; cache_creation_input_tokens?: number | null } | undefined,
): number {
  ...
  const cost = pricing
    ? ((inTok + cacheRead * 0.1 + cacheWrite * 1.25) / 1_000_000) * pricing.input + (outTok / 1_000_000) * pricing.output
    : 0;
  console.log(`[cv-cost] ${label} model=${model} input_tokens=${inTok} cache_read=${cacheRead} cache_write=${cacheWrite} output_tokens=${outTok} est_cost=$${cost.toFixed(4)}`);
  return cost;
}
...
async function handle(req: Request): Promise<Response> {
  ...
  // subscription's monthly allotment (resets each billing period, never
  // touches the one-time pack balance), (2) the original one-time
  // $49/10-credit pack (cumulative, never expires). A subscriber who has
  // exhausted this period's allotment falls through to (2) as overage,
  // exactly the "buy the existing pack as a top-up" design the
  // subscription-pricing memo recommends -- no separate metered-billing
  // product needed. creditSource is threaded through to the usage-log
  // insert below so the two pools' usage never gets counted against each
  // other (see schema_subscriptions.sql's credit_source column comment).
  //
  // FREE_MODE (2026-09-14): pricing is paused while the product gets
  // built out further per the 100-agent stakeholder review -- no real
  // subscribers exist yet, so this skips the credit/subscription check
      ...
      if (totalCredits === 0) {
        return jsonResponse({
          error: subscription
            ...
            : "This feature requires purchasing analysis credits for the Case Value Calculator.",
  ...
  try {
    ...
    const analysisSchema = {
                      ...
                      items: {
                        ...
                        items: {
                          ...
                          properties: {
                            ...
                            disputed: { type: "boolean", description: "True when the other side contests this figure. Informational only: to put a contested amount into the range, use basis 'competing' and quote each side's figure." },
                          },
      ...
      required: ["narrativeSections", "likelyOutcome", "damagesRange", "bestGuessValue", "whatIsNeededForEstimate", "issues", "requestedReliefCeiling"],
      additionalProperties: false,
    };
    ...
    const analysisStream = anthropic.messages.stream({
      ...
      // Cost (2026-09-25, Jeff: "we need to be more efficient with the use
      // of the api credits"): the instructions and the category's case-law
      // list are identical from call to call, so both are cached blocks,
      // billed at a fraction of the input rate when reused within the
      // cache window. Only the case materials are new input each time.
      ...
      messages: [{
        ...
        content: [{
          ...
          text: `Real cited precedent you may draw on (cite ONLY from this list, by exact name):\n${citationPoolText}\n`,
        }, {
          type: "text",
          text:
    ...
    const issues = Array.isArray(analysisParsed.issues) ? analysisParsed.issues.map((iss: Record<string, unknown>) => {
      ...
      if (figures.length) {
        if (amt?.basis === "competing") {
          ...
          // v4.1: an itemized claim is worth the sum of its verified
          // figures if won; the uncertainty is carried by the strength
          // band. The v4 paired runs showed a yes/no "disputed" flag moving
          // the low end between the full amount and $0 on identical input
          // (US Bank, Kotis, Dover Mall). A contested amount belongs in
          // basis 'competing', with each side's figure quoted.
          const all = figures.reduce((a, f) => a + f.value, 0);
          damagesRange = [all, all];
        }
    ...
    // The top-line case value MUST equal the sum of each issue's own
    // expected value (probability x damages) -- computed here in code,
    // not trusted from a second, independently-reasoned number the model
    // also returns. Letting those diverge is how a "holistic" case value
    // ends up bigger than what the claim-by-claim math actually supports
    // (caught in production 2026-09-16: a hand-run valuation added an
    // unsupported upward adjustment on top of its own per-claim numbers,
    // exactly the failure mode this guards against). Only issues with
    ...
    // the range exceed what the filing party actually asked the court
    // to award, regardless of what the per-issue math would otherwise
    // sum to (a stronger-than-necessary case for a $X motion is still
    // only worth $X to this specific proceeding).
    const aiDamagesRange: [number, number] | null =
      rawAiDamagesRange && requestedReliefCeiling != null
        ? [Math.min(rawAiDamagesRange[0], requestedReliefCeiling), Math.min(rawAiDamagesRange[1], requestedReliefCeiling)]
        : rawAiDamagesRange;
